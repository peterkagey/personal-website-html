...
function set_size(){
  ...
  // TODO: Fix this. Instead of 0+, this should take into account the solutionString to make sure there are enough rows.
  b_height = Math.max(0+bShift+1, default_b_height);
  setCanvasSize();
  set_r();
}
...
function number_of_vertices(){
  var count = 0;
  for (var i = 0; i < a_width*(b_height-1); i++) {
    if (labels[i] != 0){ count++; }
  }
  return count
}
...
function resize_canvas(dimension){
  ...
  if (dimension == "heighten"){
    ...
    setCanvasSize()
    initialize_atox_and_btoy();
    for(var i = a_width*(b_height-2); i < a_width*(b_height-1); i++){
      labels[i] = 0;
    }
  }

  if (dimension == "narrow"){
    for(var i = 0; i < labels.length/a_width; i++){
