...
function mainCanvas() {
  ...
  resizeBoxes()
  refreshImage()
  document.addEventListener('dragenter', function() {refreshImage()})
}
...
function explanationCanvas() {
  ...
  function drawTriangularGrid() {
    ...
    for (var row = 0; row < n - 1; row++) {
      for (var column = 0; column <= b + 1 - (n - row); column++) {
        context.beginPath();
        context.fillStyle = "#666666";
        const [x,y] = coordsToPoint(row, column);
        context.arc(x,y,scale/4,0,2*Math.PI)
        context.fill();
        context.closePath();
      }
    }
    for (row = b; row < n - 1; row++) {
      for (var c = b+1; c <= row; c++) {
        context.beginPath();
        context.fillStyle = "#666666";
        const [x,y] = coordsToPoint(row, c);
        context.arc(x,y,scale/4,0,2*Math.PI)
  ...
  function drawPositionTriangle() {
      ...
      if ((d-c) >= b) {
        const [x2, y2]   = coordsToPoint(topX + (d-c), topY)         // bottom left
        const [x21, y21] = coordsToPoint(topX + b, topY)                 // top
        const [x3, y3]   = coordsToPoint(topX + (d-c), topY + (d-c)) // bottom right
        const [x31, y31] = coordsToPoint(topX + b, topY + b)                 // top
  ...
  function drawPartitionLine() {
      const x = (coordsToPoint(n - 1, b + 1/2))[0]
      context.beginPath();
      context.setLineDash([]);
