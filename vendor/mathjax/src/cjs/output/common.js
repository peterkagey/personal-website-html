...
exports.CommonOutputJax = exports.FONTPATH = void 0;
...
exports.FONTPATH = '@mathjax/%%FONT%%-font';
var CommonOutputJax = (function (_super) {
    ...
    function CommonOutputJax(options, defaultFactory, defaultFont) {
        ...
        var _this = this;
        var _a = __read(options.fontData instanceof FontData_js_1.FontData
            ? [options.fontData.constructor, options.fontData]
            : [options.fontData || defaultFont, null], 2), fontClass = _a[0], font = _a[1];
        var _b = __read((0, Options_js_1.separateOptions)(options, fontClass.OPTIONS), 2), jaxOptions = _b[0], fontOptions = _b[1];
        _this = _super.call(this, jaxOptions) || this;
        _this.factory =
            _this.options.wrapperFactory ||
                new defaultFactory();
        _this.factory.jax = _this;
        _this.styleJson = _this.options.styleJson || new StyleJson_js_1.StyleJsonSheet();
        _this.font = font || new fontClass(fontOptions);
        _this.font.setOptions({ mathmlSpacing: _this.options.mathmlSpacing });
        _this.unknownCache = new Map();
        var linebreaks = (_this.options.linebreaks.LinebreakVisitor ||
            LinebreakVisitor_js_1.LinebreakVisitor);
    ...
    CommonOutputJax.prototype.addExtension = function (font, prefix) {
    ...
    CommonOutputJax.prototype.toDOM = function (math, node, html) {
        ...
        var makeBreaks = this.options.linebreaks.inline && !math.display;
        var inlineMarked = !!math.root.getProperty('inlineMarked');
        if (inlineMarked &&
            (!makeBreaks ||
    ...
    CommonOutputJax.prototype.getLinebreakWidth = function () {
        var W = this.math.metrics.containerWidth / this.pxPerEm;
        var width = this.math.root.attributes.get('maxwidth') ||
        ...
        try {
            for (var _b = __values(node.childNodes), _c = _b.next(); !_c.done; _c = _b.next()) {
                ...
                if (markNext) {
                    ...
                    var mo = child.coreMO();
                    var texClass = mo.texClass;
                    var linebreak = mo.attributes.get('linebreak');
                    var linebreakstyle = mo.attributes.get('linebreakstyle');
                    if ((texClass === MmlNode_js_1.TEXCLASS.BIN ||
                        texClass === MmlNode_js_1.TEXCLASS.REL ||
                        (texClass === MmlNode_js_1.TEXCLASS.ORD && mo.hasSpacingAttributes()) ||
                        linebreak !== 'auto') &&
                        linebreak !== 'nobreak') {
                    ...
                    var linebreak = child.attributes.get('linebreak');
    ...
    CommonOutputJax.prototype.markInlineBreak = function (marked, forcebreak, linebreak, node, child, mo) {
        if (mo === void 0) { mo = null; }
    ...
    CommonOutputJax.prototype.unmarkInlineBreaks = function (node) {
        var e_2, _a;
        if (!node)
            return;
        ...
        try {
            for (var _b = __values(html.math), _c = _b.next(); !_c.done; _c = _b.next()) {
                ...
                if (math.state() < MathItem_js_1.STATE.METRICS && parent_1) {
                    ...
                    var _d = map.get(parent_1), em = _d.em, ex = _d.ex, containerWidth = _d.containerWidth, scale = _d.scale, family = _d.family;
    ...
    CommonOutputJax.prototype.getMetricsFor = function (node, display) {
        ...
        var metrics = __assign(__assign({}, this.measureMetrics(test, getFamily)), { display: display });
    ...
    CommonOutputJax.prototype.getMetricMaps = function (html) {
        ...
        try {
            for (var _h = __values(maps.keys()), _j = _h.next(); !_j.done; _j = _h.next()) {
                var i = _j.value;
                try {
                    for (var _k = (e_6 = void 0, __values(domMaps[i].keys())), _l = _k.next(); !_l.done; _l = _k.next()) {
                        var node = _l.value;
        ...
        try {
            for (var _m = __values(maps.keys()), _o = _m.next(); !_o.done; _o = _m.next()) {
                var i = _o.value;
                try {
                    for (var _p = (e_8 = void 0, __values(domMaps[i].values())), _q = _p.next(); !_q.done; _q = _p.next()) {
                        var node = _q.value;
                        adaptor.remove(node);
                    }
                }
    ...
    CommonOutputJax.prototype.measureMetrics = function (node, getFamily) {
        ...
        return { em: em, ex: ex, containerWidth: containerWidth, scale: scale, family: family };
    };
    CommonOutputJax.prototype.styleSheet = function (html) {
        ...
        var sheet = this.html('style', { id: 'MJX-styles' }, [
    ...
    CommonOutputJax.prototype.insertStyles = function (_styles) { };
    ...
    CommonOutputJax.OPTIONS = __assign(__assign({}, OutputJax_js_1.AbstractOutputJax.OPTIONS), { scale: 1, minScale: .5, mtextInheritFont: false, merrorInheritFont: false, mtextFont: '', merrorFont: 'serif', mathmlSpacing: false, skipAttributes: {}, exFactor: .5, displayAlign: 'center', displayIndent: '0', displayOverflow: 'overflow', linebreaks: {
        ...
        }, font: '', htmlHDW: 'auto', wrapperFactory: null, fontData: null, fontPath: exports.FONTPATH, styleJson: null });
