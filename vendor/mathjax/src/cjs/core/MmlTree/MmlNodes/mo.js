...
var MmlMo = (function (_super) {
    ...
    MmlMo.prototype.adjustTeXclass = function (prev) {
        ...
        if (prev) {
            if (prev.getProperty('autoOP') &&
                (texClass === MmlNode_js_1.TEXCLASS.BIN || texClass === MmlNode_js_1.TEXCLASS.REL)) {
        ...
        if (texClass === MmlNode_js_1.TEXCLASS.BIN &&
            (prevClass === MmlNode_js_1.TEXCLASS.NONE ||
                prevClass === MmlNode_js_1.TEXCLASS.BIN ||
                prevClass === MmlNode_js_1.TEXCLASS.OP ||
                prevClass === MmlNode_js_1.TEXCLASS.REL ||
                prevClass === MmlNode_js_1.TEXCLASS.OPEN ||
                prevClass === MmlNode_js_1.TEXCLASS.PUNCT)) {
    ...
    MmlMo.prototype.getOperatorDef = function (mo) {
        ...
        var OPTABLE = CLASS.OPTABLE;
        var def = OPTABLE[form1][mo] || OPTABLE[form2][mo] || OPTABLE[form3][mo];
        if (def) {
            return def;
        }
        this.setProperty('noDictDef', true);
        var limits = this.attributes.get('movablelimits');
        var isOP = !!mo.match(CLASS.opPattern);
        if ((isOP || limits) && this.getProperty('texClass') === undefined) {
            return (0, OperatorDictionary_js_1.OPDEF)(1, 2, MmlNode_js_1.TEXCLASS.OP);
        }
        var range = (0, OperatorDictionary_js_1.getRange)(mo);
        var _b = __read(CLASS.MMLSPACING[range[2]], 2), l = _b[0], r = _b[1];
        return (0, OperatorDictionary_js_1.OPDEF)(l, r, range[2]);
    };
    MmlMo.prototype.checkOperatorTable = function (mo) {
        var e_1, _a;
        var def = this.getOperatorDef(mo);
        if (this.getProperty('texClass') === undefined) {
            this.texClass = def[2];
        }
        try {
    ...
    MmlMo.prototype.isMathAccent = function (mo) {
        if (mo === void 0) { mo = this.getText(); }
        var MATHACCENT = this.constructor.mathaccents;
        return !!mo.match(MATHACCENT);
    };
    MmlMo.prototype.isMathAccentWithWidth = function (mo) {
        if (mo === void 0) { mo = this.getText(); }
        var MATHACCENT = this.constructor.mathaccentsWithWidth;
        return !!mo.match(MATHACCENT);
    };
