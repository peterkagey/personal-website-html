    ...
    var TagFormat = (function (_super) {
            ...
            try {
                for (var _b = __values(Object.keys(forms)), _c = _b.next(); !_c.done; _c = _b.next()) {
                    ...
                    if (!Array.isArray(forms[form]) || forms[form].length !== 3) {
                        throw new TexError_js_1.default('InvalidTagFormDef', 'The tag form definition for "%1" should be an array of three strings', form);
                    }
        ...
        TagFormat.prototype.formatTag = function (tag) {
            if (this.mtCurrent) {
                ...
                return format
                    ? "".concat(left).concat(format, "{").concat(tag, "}").concat(right)
                    : "".concat(left).concat(tag).concat(right);
            }
